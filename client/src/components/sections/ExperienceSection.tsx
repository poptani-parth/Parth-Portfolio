...
export const ExperienceSection: React.FC<ExperienceSectionProps> = ({ experience }) => {
 return (
 <section id="experience" className="py-20 border-b border-theme-border dark:border-zinc-800/50 bg-theme-card relative transition-colors duration-200">
 ...
 {/* Timeline */}
 <div className="relative border-l border-theme-border ml-2 sm:ml-4 pl-6 sm:pl-8 space-y-10">
 ...
 :'bg-slate-300 border-slate-400 dark:border-zinc-600'
 }`}
 />
 </div>

 {/* Card */}
 <div className="bg-theme-bg dark:bg-[#0e0e0e] border border-theme-border rounded-lg p-5 sm:p-6 transition-all hover:border-slate-300 dark:hover:border-zinc-700 shadow-xs hover:shadow-md">
 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-3">
 <div>
 <div className="flex items-center gap-2 flex-wrap">
 <h3 className="text-lg font-bold text-theme-text font-['Syne',sans-serif]">
 {item.role}
 </h3>
 {item.isCurrent && (
 ...
 <p className="text-xs sm:text-sm text-theme-text-secondary mb-4 leading-relaxed font-light">
 {item.description}
 </p>
 ...
 <div key={hIdx} className="flex items-start gap-2 text-xs sm:text-sm text-theme-text-secondary">
 ...
 {/* Technologies */}
 <div className="flex flex-wrap gap-1.5 pt-3 border-t border-theme-border dark:border-zinc-900">
 {/* ADDED: Optional chaining (?.) for technologies array */}
 {item.technologies?.map((tech, tIdx) => (
 <span
 key={tIdx}
 className="text-[10px] font-sans bg-theme-card dark:bg-[#141414] text-theme-text-secondary px-2 py-0.5 rounded border border-theme-border dark:border-zinc-800/60"
 >
 {tech}
 </span>
 ))}
